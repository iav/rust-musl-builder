...
RUN echo "OPENSSL_TARGET $OPENSSL_TARGET LD_MUSL_ARCH $LD_MUSL_ARCH \
        TRIPLET $TRIPLET RUST_TARGET $RUST_TARGET RUSTUSERID $RUSTUSERID"
RUN env
...
WORKDIR /tmp
# there are workaround for linking error 
# .text.startup+0xc): undefined reference to `__getauxval'
# https://www.mail-archive.com/kvmarm@lists.cs.columbia.edu/msg27980.html
# need to use CFLAGS="-mno-outline-atomics" for gcc v10 for arm target platform
RUN curl -LO "https://www.openssl.org/source/openssl-$OPENSSL_VERSION.tar.gz" && \
	...
	GCC_MAJOR_VERSION=$(${CC} -dumpversion 2> /dev/null | cut -f1 -d.); \
	echo "CC version $GCC_MAJOR_VERSION"; \
    if [ "${GCC_MAJOR_VERSION}" -ge "10" -a "$OPENSSL_TARGET" = "linux-aarch64"  ]; \
		then export CFLAGS="-mno-outline-atomics"; echo "$CFLAGS set"; fi; \
	./Configure no-zlib no-shared -fPIC no-afalgeng \
    --prefix=$MUSL_PREFIX --openssldir=$MUSL_PREFIX/ssl \
    -DOPENSSL_NO_SECURE_MEMORY ${OPENSSL_TARGET} && \
	C_INCLUDE_PATH=$MUSL_PREFIX/include make depend && \
    C_INCLUDE_PATH=$MUSL_PREFIX make && \
...
# details about openssl vars https://docs.rs/openssl/0.10.30/openssl/#manual
    OPENSSL_DIR=$MUSL_PREFIX \
#    X86_64_UNKNOWN_LINUX_MUSL_OPENSSL_DIR=$MUSL_PREFIX \
    X86_64_UNKNOWN_LINUX_MUSL_OPENSSL_STATIC=1 \
    AARCH64_UNKNOWN_LINUX_MUSL_OPENSSL_STATIC=1 \
    ARMV7_UNKNOWN_LINUX_MUSLEABIHF_OPENSSL_STATIC=1 \
...
# manually.
RUN curl https://sh.rustup.rs -sS | \
...
ADD --chown=rust:rust cargo-config.toml /home/rust/.cargo/config
